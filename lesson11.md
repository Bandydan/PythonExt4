...
В создаваему таблицу также можно добавить значение по умолчанию для столбца. 
...
Добавить значение по умолчанию можно следующим образом:

```sql
create table cars(id serial, brand text not null default 'BMW');
CREATE TABLE
```

Важно использовать именно одинарные кавычки, чтобы команда сработала.
...
Кроме этого, существует также команда 'update table', которая изменяет значения указанных столбцов во всех строках, удовлетворяющих условию.

```sql
UPDATE cars SET brand = 'BMW' WHERE brand = 'Lada';
```
